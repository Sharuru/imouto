var iTorr = function (a, c, b) {
    ...
    b.x = function (d) {
        return function (j, f, e, h, g) {
            ...
            (g = new XMLHttpRequest()).open(f ? "POST" : "GET", j, 1);
            !f || g.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
            if (e || h) {
                g.onreadystatechange = function () {
                    if (g.readyState == 4) {
                        ((g.status > 199 && g.status < 301) || g.status == 304) ? e(d[j + (f || "")] = (g.getResponseHeader("Content-Type") || "").match(/json/) ? JSON.parse(g.responseText || "null") : g.responseText) : (!h) || h(g.status);
                    }
                };
            }
            g.send(f || "");
    ...
    b.re_date = function (g) {
        var e = new Date(), f = new Date((g + "").match(/[0-9]{10}/) ? g * 1000 : g), d = parseInt, h = d((e - f) / 1000);
...
$.onsubmit = function (c, d) {
    c.onsubmit = function () {
        for (var g = 0, h = c.length, a, b = []; g < h; g++) {
            a = c[g];
            if (a.name) {
                if (a.type == "radio") {
                    if (a.checked == 1) {
